...
Textured_3D_Shader_Program_Color::Textured_3D_Shader_Program_Color(const Shader* vertex_shader, const Shader* fragment_shader)
	: Shader_Program("Shader_Program.3D.Textured_Color", vertex_shader, fragment_shader)
{
}
Textured_3D_Shader_Program_Color::~Textured_3D_Shader_Program_Color()
{
}

void Textured_3D_Shader_Program_Color::render(const Configuration* config, const Mesh* mesh, const Texture* texture,
	...
	const glm::mat4 camera, const glm::mat4 projection) const
{
	...
	const glm::vec3 light_position(0.0, 0.0, -3.0);
	GLint light_position_location = glGetUniformLocation(program(), "light_position");
	expect(light_position_location != -1, "Failed to find light position location.");
	glUniform3fv(light_position_location, 1, (float*)(&light_position));

	//blue light
	const glm::vec3 light_color(0.0, 0.3, 1.0);
	GLint light_color_location = glGetUniformLocation(program(), "light_color");
	expect(light_color_location != -1, "Failed to find light color location.");
	glUniform3fv(light_color_location, 1, (float*)(&light_color));

	GLuint nbo_index;
	glGenBuffers(1, &nbo_index);
	glBindBuffer(GL_ARRAY_BUFFER, nbo_index);
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * mesh->normals().size(), mesh->normals().data(), GL_STATIC_DRAW);

	GLint vertex_normal_location = glGetAttribLocation(program(), "vertex_normal");
	expect(vertex_normal_location != -1, "Failed to get vertex_normal_location");
	glEnableVertexAttribArray(vertex_normal_location);
	glVertexAttribPointer(vertex_normal_location, 3, GL_FLOAT, GL_FALSE, 0, NULL);
